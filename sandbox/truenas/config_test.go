...
func TestToConfig(t *testing.T) {
	...
	if cfg.TrueNAS.Host != "nas.local" {
		t.Errorf("host = %q", cfg.TrueNAS.Host)
	}
	if cfg.TrueNAS.Port != 8443 {
		t.Errorf("port = %d", cfg.TrueNAS.Port)
	}
	if cfg.TrueNAS.Username != "admin" {
		t.Errorf("username = %q", cfg.TrueNAS.Username)
	}
	if cfg.TrueNAS.APIKey != "key-123" {
		t.Errorf("api_key = %q", cfg.TrueNAS.APIKey)
	}
	if cfg.TrueNAS.InsecureSkipVerify == nil || !*cfg.TrueNAS.InsecureSkipVerify {
		t.Error("insecure should be true")
	}
	if cfg.SSH.User != "pixel" {
		t.Errorf("ssh user = %q", cfg.SSH.User)
	}
	if cfg.SSH.Key != "/tmp/key" {
		t.Errorf("ssh key = %q", cfg.SSH.Key)
	}
}
